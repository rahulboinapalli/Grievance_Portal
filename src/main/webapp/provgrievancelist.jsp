            ...
            $("#dialog-add").dialog({
                ...
                buttons: {
                    'Save': function() {
                        ...
                        var v_type =window.document.getElementById("type").value;
...
//                        alert("v_rid:="+v_rid+" : v_npi= "+v_npi+" :v_pname= "+v_pname+" :v_type="+v_type+" :v_comments="+v_comments);
                        if(validateForm()){
                            ...
                            url:'savegrievance',
                            type:'post',
                            data: "v_rid="+v_rid+"&v_npi="+v_npi+"&v_pname="+v_pname+"&v_date="+v_date+"&v_type="+v_type+"&v_email="+v_email+"&v_phone="+v_phone+"&v_comments="+v_comments,
                            success:function(data){
                                // var response = data.responseText;
                                //                            alert("response= ::: "+response);

                                window.document.getElementById("rid").value = "";
                                window.document.getElementById("npi").value= "";
                                window.document.getElementById("pname").value = "";
                                window.document.getElementById("datepicker").value = "";
                                window.document.getElementById("type").value = "";
                                window.document.getElementById("datepicker").value = "";
                                window.document.getElementById("email").value ="";
                                window.document.getElementById("phone").value = "";
                                window.document.getElementById("comments").value="";
                                var mydata1 = [ {rid:v_rid,pnpi:v_npi,pname:v_pname,pyear:v_date,gtype:"Self",date:v_date,status:"Submitted"}];
                                for(var i=0;i<=mydata1.length;i++){
                                    jQuery("#list4").jqGrid('addRowData',i+1,mydata1[i])
                                }
